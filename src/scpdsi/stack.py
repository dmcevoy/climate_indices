'''
From https://python-algorithms.readthedocs.org/en/latest/_modules/python_algorithms/basic/stack.html
Described here: https://python-algorithms.readthedocs.org/en/latest/python_algorithms.basic.html#module-python_algorithms.basic.stack
'''
...
class Stack(object):
    ...
    def add_first(self, item):
        """Insert an item to the front of the stack."""
        if self._size > 0:

            # we have at least one element so we start at the head of the stack and work our way down
            current = self._head
            while (current.next is not None):
                current = current.next

            # current is now the first node in the stack, make that node now the second by
            # creating a new node in front of it (i.e. as the next node) containing the item
            current.next = _Node(item)
            self._size += 1
        else:
            self.push(item)

    def sorted_list(self):
        """Return a list of the items of the stack, in FIFO order
        ...
        """
        if self.isEmpty():
            raise IndexError("list of an empty stack")

        items = []
        current_node = self._head
        items_remaining = self._size
