...
if __name__ == '__main__':
    ...
    Index values are fitted to the Pearson Type III distribution.  
    
    example invocation:
    
    python monthly_spei_pearson.py C:/tmp/nclimgrid/lowres/lowres_nclimgrid_189501_201412_prcp.nc \
                                 prcp \
    ...
    try:
        ...
        with netCDF4.Dataset(precip_file) as precip_dataset, netCDF4.Dataset(temp_file) as temp_dataset:
            ...
            for month_scale_index, month_scale_var_name in enumerate(month_scales):
                ...
                variable_name = 'spei_pearson_{}'.format(str(month_scale_var_name).zfill(2))
                ...
                dataset = initialize_dataset(output_file_base + variable_name + '.nc',
                                                       precip_dataset,
                                                       x_dim_name,
                                                       y_dim_name,
                                                       variable_name,
                                                       'SPEI (Pearson), {}-month scale'.format(str(month_scale_var_name)),
                                                       variable_attributes,
                                                       np.nan)
                    ...
                    if (isinstance(precip_data, np.ma.MaskedArray)) and precip_data.mask.all():
                        ...
                        for month_scale_index, month_scale_var_name in enumerate(sorted(datasets.keys())):
                            ...
                            datasets[month_scale_var_name].variables[month_scale_var_name][:, x, y] = \
                                indices.spei_pearson(precip_data,
                                                     temp_data,
