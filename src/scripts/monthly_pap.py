...
if __name__ == '__main__':
    ...
    '''
    This program will compute PAP corresponding to a precipitation dataset at a specified month scale. Index values 
    are fitted to the Pearson type III distribution.  
    
    example invocation:
    
    python monthly_pap.py C:/tmp/nclimgrid/lowres/nclimgrid_189501_201412_prcp.nc \
                          prcp \
                          C:/tmp/nclimgrid/lowres/nclimgrid_189501_201412_ 
    ...
    try:
        ...
        with netCDF4.Dataset(precip_file) as precip_dataset:
            ...
            datasets = {}
            for month_scale_index, month_scale in enumerate(month_scales):

                # create the variable name from the indicator, distribution, and month scale
                variable_name_spi_gamma = 'pap_{}'.format(str(month_scale).zfill(2))
                ...
                dataset = netcdf_utils.initialize_dataset(output_file_base + variable_name_spi_gamma + '.nc',
                                                          precip_dataset,
                                                          x_dim_name,
                                                          y_dim_name,
                                                          variable_name_spi_gamma,
                                                          'PAP, {}-month scale'.format(str(month_scale)),
                                                          variable_attributes,
                                                          np.nan)
            
                # add to the list of month scale datasets
                datasets[variable_name_spi_gamma] = dataset
            ...
            for x in range(precip_dataset.variables[x_dim_name].size):
                for y in range(precip_dataset.variables[y_dim_name].size):
                    ...
                    if (isinstance(precip_data, np.ma.MaskedArray)) and precip_data.mask.all():
                        ...
                        for month_scale_index, month_scale in enumerate(sorted(datasets.keys())):

                            # perform the SPI computation (fit to the Gamma distribution) and assign the values into the dataset
                            datasets[month_scale].variables[month_scale][:, x, y] = \
                                indices.percentage_of_average(precip_data, month_scales[month_scale_index])
