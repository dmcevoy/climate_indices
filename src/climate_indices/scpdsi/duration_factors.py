...
def compute_duration_factors(zindex_values,
    ...
    for lon in range(zindex_values.shape[0]):
        for lat in range(zindex_values.shape[1]):
            ...
            # only compute if we have non-NaN Z-index values
            if ~np.all(np.isnan(zindex_values[lon][lat])):
                ...
                for wet_or_dry in ['wet', 'dry']:
                    ...
                    for i, month_scale_var_name in enumerate(month_scales):
    
                        try:
                            z_sums[i] = get_z_sum(month_scale_var_name,
                                                  wet_or_dry,
                ...
                logger.info("All NaN Z_Index values")
